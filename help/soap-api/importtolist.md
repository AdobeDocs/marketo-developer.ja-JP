...
| メール | 最初 | 姓 |
| --- | --- | --- |
| joe@company.com | ジョー | Smith |
| mary@company.com | メアリー | ロジャーズ |
| wanda@megacorp.com | ワンダ | ウィリアムズ |

`name` の値ではなく、`importFileHeader` で `displayName` の値を使用する必要があります。

**動的メールコンテンツ：** オプションで、メール内のマイトークンの置き換えとして機能する値をリードごとに渡すことができます。

| メール | 最初 | 姓 | {{my.specialToken}} | {{my.otherToken}} |
| --- | --- | --- | --- | --- |
| joe@company.com | ジョー | Smith | 魚 | ブルー |
| mary@company.com | メアリー | ロジャーズ | チキン | ブラウン |
| wanda@megacorp.com | ワンダ | ウィリアムズ | 野菜 | ヘーゼル |
...
完了 `importToList` るには時間がかかる場合があります（特に、リストが大きい場合）。 新しく読み込んだリストを他の API 呼び出しで使用する予定がある場合は、`importToListStatus` を使用して操作が完了していることを確認してください。

**注意：** CSV ファイルの数値フィールドに NULL 値を読み込むと、フィールドが既に空白の場合でも、これらのフィールドに「データ値を変更」アクティビティが生成される場合があります。 「データ値が変更されました」フィルターまたは「データ値が変更されました」トリガーを使用するスマートキャンペーンでは、データが実際には変化しない場合でも、リードがこれらのキャンペーンに適合する可能性があります。 これらのフィルターやトリガーに対して制約を使用して、インポートの実行時にリードが間違ったキャンペーンの対象とならないようにします。
...
| campaignName | オプション | マイトークンのオーバーライドを使用する場合、これは、これらのトークンが使用されるキャンペーンの名前です。 キャンペーンは、指定したプログラム内にある必要があります。 |
