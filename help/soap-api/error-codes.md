...
Marketoの開発では、予期しない例外が発生した場合に、リクエストと応答がログに記録されることが非常に重要です。  期限切れの認証などの特定のタイプの例外は再認証によって安全に処理できますが、他の例外ではサポートのインタラクションが必要になる場合があり、このシナリオでは常にリクエストと応答がリクエストされます。
