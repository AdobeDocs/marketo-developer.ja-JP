...
- Marketo ID
- 外部システム ID （として実装） `foreignSysPersonId`）
...
メールを除き、これらの識別子はすべて一意のキーとして扱われます。 Marketo ID は、他のすべてのキーよりも優先されます。 両方の場合 `foreignSysPersonId` が設定され、Marketo ID がリードレコードに存在する場合は、Marketo ID が優先され、 `foreignSysPersonId` そのリードに関して更新されます。 唯一の `foreignSysPersonId` が指定されると、一意の ID として使用されます。 両方の場合 `foreignSysPersonId` メールは存在するが、Marketo ID が存在しない、 `foreignSysPersonId` が優先され、そのリードのメールが更新されます。
...
| フィールド名 | 必須／オプション | 説明 |
| --- | --- | --- |
| leadRecord->Id | 必須 – メールまたは `foreignSysPersonId` が存在しない | リードレコードのMarketo ID |
| leadRecord->Email | 必須 – ID またはの場合のみ `foreignSysPersonId` が存在しない | リードレコードに関連付けられている電子メールアドレス |
| leadRecord->`foreignSysPersonId` | 必須 – ID またはメールが存在しない場合のみ | リードレコードに関連付けられた外部システム ID |
| leadRecord->foreignSysType | オプション – 次の場合のみ必須 `foreignSysPersonId` が存在する | 外部システムのタイプ。 使用可能な値：CUSTOM、SFDC、NETSUITE |
...
| marketoCookie | オプション | この [Munchkin javascript](../javascript-api/lead-tracking.md) cookie |
