...
>[!NOTE]
>2026 年 3 月 31 日以降、結合リード API 呼び出しの `leadIds` パラメーターに 25 を超える ID を含む呼び出しは、エラーコード 1080 が発生し、呼び出しはスキップされます。 25 を超えるレコードを 1 つに統合する必要があるジョブは、これらの呼び出しを確実に成功させるために複数のジョブに分割する必要があります。
>
