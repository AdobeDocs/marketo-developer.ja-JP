...
使用状況APIは、サブスクリプションのREST APIの使用とエラーアクティビティを要約します。 これらのエンドポイントを使用して、統合の監視、1日の通話量の追跡、エラーの傾向の特定を行います。
...
使用状況レスポンスは、各API ユーザーを個別にレポートします。 統合を個別のAPI ユーザーに割り当てることで、割り当てを消費し、エラーを生成するサービスを特定しやすくなります。
