...
リードを除くすべてのオブジェクトについて、対応するdescribe呼び出しの検索可能フィールドから{field to query}を選択し、最大300個の値のコンマ区切りリストを作成できます。 次のオプションのクエリパラメーターもあります。
...
リード API 以外で、リードデータベースオブジェクトを作成または更新する呼び出しでは、`result` 配列内の各オブジェクトの `seq` フィールドが返されます。 リストされる番号は、リクエスト内で更新されたレコードの順序に対応します。 各項目は、オブジェクトタイプの `idField` の値と `status` を返します。 ステータスフィールドには、「作成済み」、「更新」、「スキップ済み」のいずれかが示されます。  ステータスがスキップされた場合、コードとメッセージを含む1つ以上の理由オブジェクトを持つ対応する「reasons」配列も存在し、レコードがスキップされた理由を示します。 詳しくは、[エラーコード](error-codes.md)を参照してください。
