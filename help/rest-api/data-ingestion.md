...
| `dedupeFields` | オブジェクト | いいえ | 重複排除する属性。 1つまたは2つの属性名を使用できます。<br/> AND 操作では、2 つの属性が使用されます。 例えば、`email`と`firstName`の両方が指定されている場合、両方ともAND操作を使用してユーザーを検索するために使用されます。<br/>サポートされる属性は、`id`、`email`、`sfdcAccountId`、`sfdcContactId`、`sfdcLeadId`、`sfdcLeadOwnerId`です。カスタム属性（「文字列」および「整数」タイプのみ）、`email`です |  |
...
| dedupeBy + action | `createOnly`と`createOrUpdate`は`dedupeFields`のみを許可します。`updateOnly` `dedupeFields`と`idField`の両方を許可します。 |
